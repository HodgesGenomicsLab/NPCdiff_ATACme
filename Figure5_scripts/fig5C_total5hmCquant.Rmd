...
total_hmCsum<- modC_cluster_hmCsum %>% group_by(time) %>%summarise(t_hmCA = mean(hmCA), t_hmCB = mean(hmCB))
total_hmC_boxplot<- ggplot()+
  geom_boxplot(data = filter(modC_cluster, cluster != "static"), aes(x= time, y= reg_avg_hmC, fill = time, color = time),outlier.shape = NA, alpha = 0.75)+
  ...
  theme_minimal()
total_hmC_boxplot
ggsave(total_hmC_boxplot, filename= "/Users/lindsey/Desktop/Hodges_Lab/ATAC-me\ NPCdiff/NPCDiff_ATACme/New_timepoints/Figures/total_hmC_time_6L.pdf", units = "in", width = 3, height = 5)

#values are replicate regional averages 
hmC<- c(0.0760, 0.0732, 0.0901, 0.0944, 0.0806, 0.0835)
rep<- c("A", "B", "A", "B","A", "B")
time<- c("0day", "0day", "4day", "4day", "8day", "8day")
stat_t<- as.data.frame(cbind(time,rep,hmC))
anova<- aov(hmC ~ time, 
    data= stat_t)
res<- summary(anova)
res
