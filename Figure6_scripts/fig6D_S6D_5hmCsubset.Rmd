...
```
```{r}
modC_cluster$time<- factor(modC_cluster$time, levels = c("0", "96", "192"))
modC_cluster_coord<- inner_join(modC_cluster, peaks)
hmC_high_0day<- modC_cluster_coord %>% pivot_wider(id_cols = c(peakID, cluster, region_chr, region_start, region_end), values_from = reg_avg_hmC, names_from = time) %>% filter(`0` >= 0.25)  %>% ungroup() %>% select(-`96`, -`192`, -peakID, -cluster) %>% drop_na() 
hmC_low_0day <- modC_cluster_coord %>% pivot_wider(id_cols = c(peakID, cluster, region_chr, region_start, region_end), values_from = reg_avg_hmC, names_from = time) %>% filter(`0` < 0.25)  %>% ungroup() %>% select(-`96`, -`192`, -peakID, -cluster) %>% drop_na() 

hmC_high_4day<- modC_cluster_coord %>% pivot_wider(id_cols = c(peakID, cluster, region_chr, region_start, region_end), values_from = reg_avg_hmC, names_from = time) %>% filter(`96` >= 0.25)  %>% ungroup() %>% select(-`0`, -`192`, -peakID, -cluster) %>% drop_na() 
hmC_low_4day <- modC_cluster_coord %>% pivot_wider(id_cols = c(peakID, cluster, region_chr, region_start, region_end), values_from = reg_avg_hmC, names_from = time) %>% filter(`96` < 0.25)  %>% ungroup() %>% select(-`0`, -`192`, -peakID, -cluster) %>% drop_na() 

hmC_high_8day<- modC_cluster_coord %>% pivot_wider(id_cols = c(peakID, cluster, region_chr, region_start, region_end), values_from = reg_avg_hmC, names_from = time) %>% filter(`192` >= 0.25)  %>% ungroup() %>% select(-`0`, -`96`, -peakID, -cluster) %>% drop_na()
hmC_low_8day <- modC_cluster_coord %>% pivot_wider(id_cols = c(peakID, cluster, region_chr, region_start, region_end), values_from = reg_avg_hmC, names_from = time) %>% filter(`192` < 0.25)  %>% ungroup() %>% select(-`0`, -`96`, -peakID, -cluster) %>% drop_na()

write_tsv(hmC_high_0day, "/Users/lindsey/Desktop/Hodges_Lab/ATAC-me\ NPCdiff/NPCDiff_ATACme/New_timepoints/data/6l_seq/hmC_high_0day.bed")
write_tsv(hmC_low_0day, "/Users/lindsey/Desktop/Hodges_Lab/ATAC-me\ NPCdiff/NPCDiff_ATACme/New_timepoints/data/6l_seq/hmC_low_0day.bed")
write_tsv(hmC_high_4day, "/Users/lindsey/Desktop/Hodges_Lab/ATAC-me\ NPCdiff/NPCDiff_ATACme/New_timepoints/data/6l_seq/hmC_high_4day.bed")
write_tsv(hmC_low_4day, "/Users/lindsey/Desktop/Hodges_Lab/ATAC-me\ NPCdiff/NPCDiff_ATACme/New_timepoints/data/6l_seq/hmC_low_4day.bed")
write_tsv(hmC_high_8day, "/Users/lindsey/Desktop/Hodges_Lab/ATAC-me\ NPCdiff/NPCDiff_ATACme/New_timepoints/data/6l_seq/hmC_high_8day.bed")
write_tsv(hmC_low_8day, "/Users/lindsey/Desktop/Hodges_Lab/ATAC-me\ NPCdiff/NPCDiff_ATACme/New_timepoints/data/6l_seq/hmC_low_8day.bed")


modC_hmC_subsets<- modC_cluster %>% mutate(hmC_level = case_when(reg_avg_hmC >=0.25 ~ "high", 
                            reg_avg_hmC < 0.25 ~ "low"))

modC_hmC_subsets$cluster<- factor(modC_hmC_subsets$cluster, levels = c(3,1,2,6,4,7,5, "static"))

hmC_barplot_subsets<- ggplot(filter(modC_hmC_subsets, cluster != "static"), aes(x= hmC_level, color = cluster, fill = cluster))+
...
ggsave(hmC_barplot_subsets, filename= "/Users/lindsey/Desktop/Hodges_Lab/ATAC-me\ NPCdiff/NPCDiff_ATACme/New_timepoints/Figures/hmCsubset_clusterdist_time_6L.pdf", units = "in", width = 6, height = 5)

hmC_barplot_subsets_all<- ggplot(filter(modC_hmC_subsets), aes(x= hmC_level, color = cluster, fill = cluster))+
  geom_bar(position = "fill")+
  scale_fill_manual(values=met.brewer("Renoir", 8))+
  scale_color_manual(values=met.brewer("Renoir", 8))+
  facet_wrap(~time)+
  theme_minimal()
hmC_barplot_subsets_all
ggsave(hmC_barplot_subsets, filename= "/Users/lindsey/Desktop/Hodges_Lab/ATAC-me\ NPCdiff/NPCDiff_ATACme/New_timepoints/Figures/hmCsubset_clusterdist_time_6L_wstatic.pdf", units = "in", width = 6, height = 5)
